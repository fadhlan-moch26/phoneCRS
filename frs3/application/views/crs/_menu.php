<div class="col-md-12" style="background:url(assets/global/img/background.jpg); opacity:0.7; height:500px;">
    <div class="note note-bordered note-danger" style="text-align:center">
        <h4>Ini adalah aplikasi yang akan membantu anda mendapatkan handphone berdasarkan kebutuhan anda. 
		Kami sajikan dua macam cara model interaksi untuk pencarian produk yang anda inginkan. 
		Model A adalah pencarian handphone berdasarkan kebutuhan fungsional yang anda inginkan,
		Model B adalah pencarian produk berdasarkan spesifikasi teknis yang anda inginkan</h4>
        <h4>Silahkan pilih salah satu jenis dialog pertanyaan berikut ini</h4>
        <br>
        <div class="tiles">
        	<div class="tile double" style="cursor:default">
            </div>
        	<div class="tile double  bg-red"><a href="javascript:u1()">
                <div class="tile-body">
                    <i class="fa fa-calendar"></i>
                </div>
                <div class="tile-object">
                    <div class="name">
                         Model A
                    </div>
                </div>
            </a></div>
        	<div class="tile double  bg-red"><a href="javascript:qsr()">
                <div class="tile-body">
                    <i class="fa fa-calendar"></i>
                </div>
                <div class="tile-object">
                    <div class="name">
                         Model B
                    </div>
                </div>
            </a></div>            
        </div>      
    </div>
</div>